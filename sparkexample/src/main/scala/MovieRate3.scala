...
object MovieRate3 {
  ...
  def main(args: Array[String]): Unit = {
    //1.加载Spark配置环境
    val conf = new SparkConf().setAppName("AnalyzeMovie3").setMaster("local")
    //2. 获取Spark上下文对象
    val sc = new SparkContext(conf);

    val inputFile = "src\\input\\moviemei.csv"
    val outputFile = "src\\main\\resources\\static\\data\\movierate3.json"
    //3. 读取存放用户信息的文件
