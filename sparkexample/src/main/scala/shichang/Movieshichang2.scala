...
object Movieshichang2 {
  def main(args: Array[String]): Unit = {
    ...
    val inputFile = "src\\doubanzhong.csv"
    val outputFile = "src\\main\\resources\\static\\data\\movieshi2.json"
    //3. 读取存放电影信息的文件
    val csv = sc.textFile(inputFile)
    val clean= csv.filter(x=>{
      val result :Array[String] =x.split(",")
      result.length>=16 && !result.exists(x=>x.isEmpty)
